...
static s16 MEMNODE_setPrev(MemoryNode *node, MemoryNode *prev_node);
...
MemoryNode* MEMNODE_next(MemoryNode *node) // returns next node
{
  if(NULL == node){
#ifdef VERBOSE_
      printf("Error: [%s] The pointer to memory node is null\n", __FUNCTION__);
...
MemoryNode* MEMNODE_prev(MemoryNode *node) // returns next node
{
  if(NULL == node){
#ifdef VERBOSE_
      printf("Error: [%s] The pointer to memory node is null\n", __FUNCTION__);
...
s16 MEMNODE_setPrev(MemoryNode *node, MemoryNode *prev_node)
{
  if(NULL == node){
#ifdef VERBOSE_
      printf("Error: [%s] The pointer to memory node is null\n", __FUNCTION__);
