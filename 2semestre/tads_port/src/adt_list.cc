...
int_least16_t List::reset()
{
  //list->first_ shouldn't be NULL if list isn't empty but to be safe we check
  //it
  if (!isEmpty() && nullptr != first_) {
...
int_least16_t List::resize(const u16 new_size)
{
  if (new_size < length_) {
...
uint16_t List::capacity() const
{
  return capacity_;
}

uint16_t List::length() const
{
  return length_;
}
...
int_least16_t List::insertFirst(void* data, const u16 data_size)
{
  if (nullptr == data) {
#ifdef VERBOSE_
...
int_least16_t List::insertLast(void* data, const u16 data_size)
{
  if (nullptr == data) {
#ifdef VERBOSE_
    printf("Error: [%s] the data passed is null\n", __FUNCTION__);
...
int_least16_t List::insertAt(void* data, const u16 position, const u16 data_size)
{
  if (nullptr == data) {
#ifdef VERBOSE_
    printf("Error: [%s] the data passed is null\n", __FUNCTION__);
...
int_least16_t List::concat(List* src)
{
  if (nullptr == src) {
#ifdef VERBOSE_ 
...
uint16_t List::traverse(s16 ( MemoryNode::* callback)()) const
{
  u16 index = 0;
  MemoryNode *aux = first_;
  while (nullptr != aux) {
